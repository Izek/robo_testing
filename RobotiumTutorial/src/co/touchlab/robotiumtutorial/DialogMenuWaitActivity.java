...
/**
 * This activity contains a Menu with various options for displaying widgets and
 * navigation All menu options are performed after some delay, via a Handler, to
 * test the Robotium "wait" methods
 */
public class DialogMenuWaitActivity extends Activity {
	...
	public void showDialog(MenuItem menuItem) {
		...
		// The default behavior of dialogs made with a Builder class are to
		...
		// Once the dialog is shown, we can get the button directly from the
		// dialog and overwrite the onClick()
	...
	public void startActivity(MenuItem menuItem) {
		delayHandler.postDelayed(new Runnable() {
			@Override
			public void run() {
				startActivity(new Intent(DialogMenuWaitActivity.this,
						DummyActivity.class));
			}
		}, 4000);
	}
